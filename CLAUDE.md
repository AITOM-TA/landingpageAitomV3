...
└── Nav.astro         ← Fixed nav (68px), mobile burger; links: #solution #fonctionnement #faq #contact
...
- **Scroll reveal:** elements opt in with `data-animate`; an IntersectionObserver in `Layout.astro`'s script adds `.is-visible`, which triggers a `@keyframes reveal` animation (translateY + scale + blur, 950ms). Keyframes are used deliberately so the reveal doesn't fight hover `transition`s on cards. Stagger with `data-delay="1"`–`"5"` (90–480ms). One-shot: the observer `unobserve`s after reveal.
